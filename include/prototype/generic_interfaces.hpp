...
private:
    ...
    struct future {
        ...
        void wait() {
            ...
            idx_neigh = 0;
            std::for_each(m_neighbors_list.begin(), m_neighbors_list.end(),
                          [this, &idx1D, &idx_neigh, &r_size] (auto const& neighbor) {
                auto r = m_iteration_space(m_id, neighbor.id(), neighbor.direction());
                r_size = std::tuple_size<decltype(r)>::value;
                idx1D = 0;
                gridtools::range_loop(r, [this, r_size, &idx1D, &idx_neigh] (auto const& indices) {
                    auto value = (*(m_data_ptrs[idx_neigh]))[idx1D++];
                    m_data_desc.set_data(value, indices);
                });
                ++idx_neigh;
            });
...
public:
    ...
    struct future {
        ...
        void wait() {
            ...
            gettimeofday(&wait_start, nullptr);
            ...
            struct timeval wait_stop;
            gettimeofday(&wait_stop, nullptr);
            double wait_lapse = ((static_cast<double>(wait_stop.tv_sec) + 1 / 1000000.0 * static_cast<double>(wait_stop.tv_usec)) -
                                 (static_cast<double>(wait_start.tv_sec) + 1 / 1000000.0 * static_cast<double>(wait_start.tv_usec))) * 1000.0;
            m_times.push_back(std::make_pair("wait", wait_lapse));
